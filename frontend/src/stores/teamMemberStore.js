  ...
  actions: {
    async fetchMembers() {
      return run(this, async () => {
        const { data } = await apiClient.get('/team_members')
        this.members = data.team_members ?? []
        return this.members
      })
    },
    async createMember(payload) {
      return run(this, async () => {
        const { data } = await apiClient.post('/team_members', { team_member: payload })
    ...
    async toggleActive(id, active) {
      return run(this, async () => {
        const { data } = await apiClient.patch(`/team_members/${id}`, { team_member: { active } })
        const index = this.members.findIndex((member) => member.id === data.id)
        if (index !== -1) this.members.splice(index, 1, data)
        return data
      })
    }
  }
