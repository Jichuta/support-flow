...
async function submitComment() {
  ...
  if (!form.team_member_id) {
    formError.value = 'Choose a comment author.'
    return
  }
  if (form.body.trim().length < 10) {
    formError.value = 'Comment must be at least 10 characters.'
    return
  }
  submitting.value = true
  try {
    await requestStore.addComment(props.id, {
...
async function loadDetail() {
  ...
  try {
    await Promise.all([requestStore.fetchRequest(props.id), memberStore.fetchMembers()])
  } catch (err) {
...
<template>
  <section class="detail-page">
    <div class="detail-actions">
      <RouterLink to="/requests"> &lt; Back to List</RouterLink
      ><RouterLink v-if="request" :to="`/requests/${request.id}/edit`" class="button"
        >Edit Request</RouterLink
      >
    </div>
    <div v-if="requestStore.loading && !request" class="loading" aria-live="polite">
      Loading support request?
    </div>
    <div v-else-if="notFound" class="state">
      <h2>Request not found</h2>
      <p>This support request may have been removed or the link is invalid.</p>
      <RouterLink to="/requests">Return to requests</RouterLink>
    </div>
    <div v-else-if="requestStore.error" class="state error" role="alert">
      <h2>Unable to load request</h2>
      <p>{{ requestStore.error.message }}</p>
      <button @click="requestStore.fetchRequest(id)">Try again</button>
    </div>
    <template v-else-if="request">
      <article class="request-card">
        <div class="request-heading">
          ...
          <div class="badges">
            <StatusBadge :status="request.status" /><PriorityBadge :priority="request.priority" />
          </div>
        </div>
        ...
        <dl class="metadata">
          ...
          <div>
            <dt>Due date</dt>
            <dd>{{ formatDate(request.due_date) }}</dd>
          </div>
          <div>
            <dt>Created</dt>
      ...
      <div class="comments-grid">
        <section class="comments-card">
          ...
          <div v-if="comments.length" class="timeline">
