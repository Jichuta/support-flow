...
async function submit() {
  ...
  try {
    ...
    mapBackendErrors(error.details)
  } finally {
    loading.value = false
  }
}
  ...
  try {
    ...
    if (isEdit.value) {
      ...
      Object.assign(form, {
        ...
        assignee_id: request.assignee?.id || ''
      })
    }
  } catch (error) {
    mapBackendErrors(error.details || [error.message])
  } finally {
    loading.value = false
    ready.value = true
  }
...
<template>
  <section class="form-page">
    <header>
      <h2>{{ isEdit ? 'Edit Support Request' : 'New Support Request' }}</h2>
      <p>Provide the details needed to track this request.</p>
    </header>
    <form v-if="ready" class="request-form" novalidate @submit.prevent="submit">
      <p v-if="errors.general" class="form-error" role="alert">{{ errors.general.join(' ') }}</p>
      <label
        >Title<input v-model="form.title" :disabled="loading" /><span
          v-if="errors.title"
          class="field-error"
          role="alert"
          >{{ errors.title[0] }}</span
      ...
      >
      <label
        >Description<textarea v-model="form.description" rows="6" :disabled="loading" /><span
      ...
      <div class="form-grid">
        <label
          >Priority<select v-model="form.priority" :disabled="loading">
            <option value="low">Low</option>
            <option value="medium">Medium</option>
            <option value="high">High</option>
            <option value="critical">Critical</option></select
          ><span v-if="errors.priority" class="field-error" role="alert">{{
            errors.priority[0]
          }}</span></label
        ><label>Due Date<input v-model="form.due_date" type="date" :disabled="loading" /></label
        ><label
          >Assignee<select v-model="form.assignee_id" :disabled="loading">
            <option value="">Unassigned</option>
            <option v-for="member in activeMembers" :key="member.id" :value="member.id">
              {{ member.name }}
            </option></select
          ><span v-if="errors.assignee_id" class="field-error" role="alert">{{
            errors.assignee_id[0]
          }}</span></label
        >
      </div>
      <div v-if="!isEdit" class="form-grid required-context">
        <label
          >Creator<select v-model="form.creator_id" :disabled="loading">
    ...
    <p v-else class="loading">Loading form...</p>
